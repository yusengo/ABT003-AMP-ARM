...
void MBuart_init(uint32 baud)
{
    ...
    //设置的时候，应该禁止发送接受
    UART_C2_REG(UARTN[MB_RXTX_PORT]) &= ~(0
                                   | UART_C2_TE_MASK
                                   | UART_C2_RE_MASK
                                  );
    ...
    if(sbr > 0x1FFF)
	sbr = 0x1FFF;                                       //SBR 是 13bit，最大为 0x1FFF
    ...
    //写 SBR
    temp = UART_BDH_REG(UARTN[MB_RXTX_PORT]) & (~UART_BDH_SBR_MASK);           //缓存 清空 SBR 的 UARTx_BDH的值
    UART_BDH_REG(UARTN[MB_RXTX_PORT]) = temp |  UART_BDH_SBR(sbr >> 8);        //先写入SBR高位
    UART_BDL_REG(UARTN[MB_RXTX_PORT]) = UART_BDL_SBR(sbr);                     //再写入SBR低位

    //写 BRFD
    temp = UART_C4_REG(UARTN[MB_RXTX_PORT]) & (~UART_C4_BRFA_MASK) ;           //缓存 清空 BRFA 的 UARTx_C4 的值
    UART_C4_REG(UARTN[MB_RXTX_PORT]) = temp |  UART_C4_BRFA(brfa);             //写入BRFA



    //设置FIFO(FIFO的深度是由硬件决定的，软件不能设置)
    UART_PFIFO_REG(UARTN[MB_RXTX_PORT]) |= (0
